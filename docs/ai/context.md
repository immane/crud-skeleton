...
`cancel` is enabled from `[draft, pending, confirmed, store_rejected]`; `refund` is paid-only (`paid → refunded`). `store_rejected` is cancellable by the customer or manager (not terminal).
